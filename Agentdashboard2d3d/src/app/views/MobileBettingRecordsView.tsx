...
interface MobileBettingRecordsViewProps {
...
export function MobileBettingRecordsView({ records, onBack, onDeleteRecord }: MobileBettingRecordsViewProps) {
  ...
  return (
    <div className="w-full max-w-[375px] h-[812px] bg-[#F5F7FA] flex flex-col mx-auto overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 px-4 py-3 shadow-lg flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-purple-100">Betting Records</p>
          </div>
        </div>
      </div>
        ...
        {records.length === 0 ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-base text-gray-500 mt-1" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
                ဇယားဝယ်မရှိပါ
              </p>
            </div>
          </div>
        ) : (
          /* Record Cards - Single Column */
                ...
                >
                  ...
                  <div className="px-3 py-3">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-base font-bold text-gray-900">{record.playerName}</h3>
                      
                      {/* Global Expand/Collapse Button - Header Position */}
                      <motion.button
                        whileTap={{ scale: 0.95 }}
                        onClick={() => toggleExpandAll(record.id, groupedBets.length)}
                        className={`px-2 py-1 text-[9px] font-bold rounded border transition-all flex items-center gap-1 ${
                          expandAllStates[record.id]
                            ? 'bg-purple-600 text-white border-purple-600'
                            : 'bg-white text-purple-600 border-purple-300'
                        }`}
                        style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif', touchAction: 'manipulation' }}
                      >
                        {expandAllStates[record.id] ? (
                          <>
                    ...
                    <div className="space-y-1.5">
                      {groupedBets.map((group, groupIndex) => {
                        ...
                        return (
                          <div key={groupIndex} className="bg-white border border-gray-300 rounded-lg overflow-hidden shadow-sm">
                            {/* Collapsible Summary Row */}
                            <button
                              onClick={() => toggleGroup(record.id, groupIndex)}
                              className="w-full px-3 py-2.5 flex items-center gap-2 bg-white active:bg-gray-50 transition-colors"
                            ...
                            >
                              ...
                              {/* Summary Text */}
                              <span className="text-sm font-extrabold text-gray-900 flex-1 text-left" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
                            ...
                            <AnimatePresence>
                              {isExpanded && (
                                ...
                                >
                                  <div className="px-2.5 py-2.5 bg-gradient-to-br from-slate-50 to-blue-50 border-t border-gray-200">
                                    {/* 5-Column Grid Layout - Clean, No Individual Borders */}
                                    <div className="grid grid-cols-5 gap-1.5 mb-2">
                                      {group.bets.map((bet, betIndex) => (
                                        <div key={betIndex} className="flex flex-col items-center justify-center py-1.5 px-0.5 bg-white rounded shadow-sm">
                                    ...
                                    {/* Total Summary */}
                                    <div className="pt-1.5 border-t border-dashed border-gray-300 flex items-center justify-between">
                                      <span className="text-[10px] font-bold text-gray-700" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
                                        စုစုပေါင်း:
                                      </span>
                  ...
                  {/* Footer - Total */}
                  <div className="px-3 py-2 border-t border-dashed border-gray-300 bg-blue-50">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-gray-800" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
                        စုစုပေါင်း
                      </span>
          ...
          >
            ...
            >
              ...
              <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-base font-bold text-gray-900" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
              ...
              <div className="px-4 py-3 overflow-y-auto max-h-[calc(90vh-140px)] hide-scrollbar">
                <h3 className="text-base font-bold text-gray-900 mb-2">{selectedRecord.playerName}</h3>
                ...
                <div className="mt-4 pt-3 border-t-2 border-dashed border-gray-300">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-gray-900" style={{ fontFamily: 'Pyidaungsu, Myanmar Text, sans-serif' }}>
                      စုစုပေါင်း:
                    </span>
                    <span className="text-base font-bold text-blue-600">
                      = {formatNumber(selectedRecord.total)} ks
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-4 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-center gap-8">
                <button
                  onClick={handleCopyAllBets}
                  className="flex flex-col items-center gap-1 p-2 active:bg-gray-100 rounded-lg transition-colors"
                  style={{ touchAction: 'manipulation' }}
                >
                ...
                <button
                  ...
                  className="flex flex-col items-center gap-1 p-2 active:bg-green-50 rounded-lg transition-colors"
                  style={{ touchAction: 'manipulation' }}
                >
                  <Printer className="w-5 h-5 text-green-600" />
                  <span className="text-[10px] text-green-600">Print</span>
                </button>
                <button
                  onClick={() => setSelectedRecord(null)}
                  className="flex flex-col items-center gap-1 p-2 active:bg-red-50 rounded-lg transition-colors"
                  style={{ touchAction: 'manipulation' }}
                >
                  <X className="w-5 h-5 text-red-600" />
                  <span className="text-[10px] text-red-600">Close</span>
                </button>
              </div>
            </motion.div>
          </motion.div>
